...
VS_OUTPUT vs_main(VS_INPUT Input)
{
   ...
   //ViewDirection
	float3 wsViewDir = normalize(wsPos.xyz - vViewPos.xyz/*-vViewDir.xyz*/);

	float3 diffuse = float3(1, 0, 0);
	float3 specular = float3(1, 0, 0);
	//dir light
	{
		//Light
...
PS_OUTPUT ps_main(VS_OUTPUT Input)
{
	...
	//dir light
	{
		...
		//Diffuse Ligth
		float3 diffuse2 = float3(1, 0, 0) * Ndl;
		float3 specular2 = float3(0, 0, 1);
		diffuse += diffuse2;
...
#if  defined(PIXEL_LIGHT) 
	Output.color = float4(diffuse.xyz + specular.xyz, 1);
#endif

#if	defined(NORMAL_MAP_LIGHT)
